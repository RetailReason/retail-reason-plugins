...
fail with an authorization error, check that `WADV_LICENSE_KEY` was set in the environment Codex
launched from and that the key has not expired or been revoked. Keys are never shared between users.
