...
This plugin connects to the hosted Walmart Advisory Service over MCP. The expertise lives
on the server; this shim's job is to route questions well and integrate the answers into the
user's work. A licensed seat key is required — if calls fail with an authorization error, tell
the user to check the plugin's license key setting (see the repo README).
